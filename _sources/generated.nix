# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  agentgateway-aarch64-darwin = {
    pname = "agentgateway-aarch64-darwin";
    version = "1.5.0";
    src = fetchurl {
      url = "https://github.com/agentgateway/agentgateway/releases/download/v1.5.0/agentgateway-darwin-arm64";
      sha256 = "sha256-2kMtNb1pbaBWT3sra7x4NUK2ucYW1sDE1MPa7536EaE=";
    };
  };
  agentgateway-aarch64-linux = {
    pname = "agentgateway-aarch64-linux";
    version = "1.5.0";
    src = fetchurl {
      url = "https://github.com/agentgateway/agentgateway/releases/download/v1.5.0/agentgateway-linux-arm64";
      sha256 = "sha256-YfEtu5lmmqS5e4WgBAGD/ksJj6CoKow4lmX+YGUXwT4=";
    };
  };
  agentgateway-x86_64-linux = {
    pname = "agentgateway-x86_64-linux";
    version = "1.5.0";
    src = fetchurl {
      url = "https://github.com/agentgateway/agentgateway/releases/download/v1.5.0/agentgateway-linux-amd64";
      sha256 = "sha256-2spc2nboxasMGnWRL+zy1jZQlUA/gQ23ICnEnRSjfns=";
    };
  };
  buzz-desktop-aarch64-darwin = {
    pname = "buzz-desktop-aarch64-darwin";
    version = "0.5.23";
    src = fetchurl {
      url = "https://github.com/block/buzz/releases/download/desktop-v0.5.23/Buzz_0.5.23_aarch64.dmg";
      sha256 = "sha256-kZfd4poJred/Vmd+B8tNap19GmoVfXIS8KBQFEBZxbI=";
    };
  };
  claude-code-aarch64-darwin = {
    pname = "claude-code-aarch64-darwin";
    version = "2.1.280";
    src = fetchurl {
      url = "https://github.com/anthropics/claude-code/releases/download/v2.1.280/claude-darwin-arm64.tar.gz";
      sha256 = "sha256-hyofofdwFlICv6DhdxqLAs4KE7X23k9EiexP8miUa1Y=";
    };
  };
  claude-code-aarch64-linux = {
    pname = "claude-code-aarch64-linux";
    version = "2.1.280";
    src = fetchurl {
      url = "https://github.com/anthropics/claude-code/releases/download/v2.1.280/claude-linux-arm64-musl.tar.gz";
      sha256 = "sha256-Ol6+QbeNlOor+v7lOd3kyaU6/JZYq7xy2RjEVHdSSyg=";
    };
  };
  claude-code-x86_64-linux = {
    pname = "claude-code-x86_64-linux";
    version = "2.1.280";
    src = fetchurl {
      url = "https://github.com/anthropics/claude-code/releases/download/v2.1.280/claude-linux-x64-musl.tar.gz";
      sha256 = "sha256-NeNDUwXlYIq6xybKLO0fGmUZgN/0MJlgZijBCkTQUQE=";
    };
  };
  cli-proxy-api-aarch64-darwin = {
    pname = "cli-proxy-api-aarch64-darwin";
    version = "7.3.14";
    src = fetchurl {
      url = "https://github.com/router-for-me/CLIProxyAPI/releases/download/v7.3.14/CLIProxyAPI_7.3.14_darwin_aarch64.tar.gz";
      sha256 = "sha256-7EXP+4gu+U8sgLiXdns3b+pX4CX/KM7xGEU9WNTIOWQ=";
    };
  };
  cli-proxy-api-aarch64-linux = {
    pname = "cli-proxy-api-aarch64-linux";
    version = "7.3.14";
    src = fetchurl {
      url = "https://github.com/router-for-me/CLIProxyAPI/releases/download/v7.3.14/CLIProxyAPI_7.3.14_linux_aarch64.tar.gz";
      sha256 = "sha256-dB65KWQQ57Z992qo0zvQYpfdaUyCKYOhpy7lSwbn9k0=";
    };
  };
  cli-proxy-api-x86_64-linux = {
    pname = "cli-proxy-api-x86_64-linux";
    version = "7.3.14";
    src = fetchurl {
      url = "https://github.com/router-for-me/CLIProxyAPI/releases/download/v7.3.14/CLIProxyAPI_7.3.14_linux_amd64.tar.gz";
      sha256 = "sha256-qTzfsui2c+s2Le2w8DF607r01esiVtopcjm623PB0j0=";
    };
  };
  codex-aarch64-darwin = {
    pname = "codex-aarch64-darwin";
    version = "0.156.0";
    src = fetchurl {
      url = "https://github.com/openai/codex/releases/download/rust-v0.156.0/codex-aarch64-apple-darwin.tar.gz";
      sha256 = "sha256-MNrg5VN2invFZM+cQtbmGdJOiVZevm0bxYnoWMogKsM=";
    };
  };
  codex-aarch64-linux = {
    pname = "codex-aarch64-linux";
    version = "0.156.0";
    src = fetchurl {
      url = "https://github.com/openai/codex/releases/download/rust-v0.156.0/codex-aarch64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-d9F6ZyYXp/03d8qdLt/9wMzRkJB4gb/z+0rtSyFKxaI=";
    };
  };
  codex-code-mode-host-aarch64-darwin = {
    pname = "codex-code-mode-host-aarch64-darwin";
    version = "0.156.0";
    src = fetchurl {
      url = "https://github.com/openai/codex/releases/download/rust-v0.156.0/codex-code-mode-host-aarch64-apple-darwin.tar.gz";
      sha256 = "sha256-d+R+nwCCBWay1uJb/MGJDBpusQqKCwoXjcwLNxJ3Oe4=";
    };
  };
  codex-code-mode-host-aarch64-linux = {
    pname = "codex-code-mode-host-aarch64-linux";
    version = "0.156.0";
    src = fetchurl {
      url = "https://github.com/openai/codex/releases/download/rust-v0.156.0/codex-code-mode-host-aarch64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-2CZaReIHs0H9XnImKOGZrFLdNUNaZ86j5eokZCD45rA=";
    };
  };
  codex-code-mode-host-x86_64-linux = {
    pname = "codex-code-mode-host-x86_64-linux";
    version = "0.156.0";
    src = fetchurl {
      url = "https://github.com/openai/codex/releases/download/rust-v0.156.0/codex-code-mode-host-x86_64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-g4PHmwtrT3fwGhylSP4dVQrZam6RufwRgLtppEnK5Ts=";
    };
  };
  codex-x86_64-linux = {
    pname = "codex-x86_64-linux";
    version = "0.156.0";
    src = fetchurl {
      url = "https://github.com/openai/codex/releases/download/rust-v0.156.0/codex-x86_64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-PUnZryWlFoz8UeUOUgqyOLIwg8JZrnwU+JsAfLJUXHs=";
    };
  };
  grok-aarch64-darwin = {
    pname = "grok-aarch64-darwin";
    version = "1.0.41";
    src = fetchurl {
      url = "https://storage.googleapis.com/grok-build-public-artifacts/cli/grok-1.0.41-macos-aarch64";
      sha256 = "sha256-nIROsTNlGAeH2a0isrN0igJL6OHthFJTzBFHgbMcWR0=";
    };
  };
  grok-aarch64-linux = {
    pname = "grok-aarch64-linux";
    version = "1.0.41";
    src = fetchurl {
      url = "https://storage.googleapis.com/grok-build-public-artifacts/cli/grok-1.0.41-linux-aarch64";
      sha256 = "sha256-fAuMlzr2p44gN/Ge2TAzRxuMXnIvn/BLhrCS4GbmDXQ=";
    };
  };
  grok-x86_64-linux = {
    pname = "grok-x86_64-linux";
    version = "1.0.41";
    src = fetchurl {
      url = "https://storage.googleapis.com/grok-build-public-artifacts/cli/grok-1.0.41-linux-x86_64";
      sha256 = "sha256-nOA+0j4W6gEHK0SWJj1iE6J4meHj4QfwCNNu34LnBAc=";
    };
  };
  llama-cpp-aarch64-darwin = {
    pname = "llama-cpp-aarch64-darwin";
    version = "11115";
    src = fetchurl {
      url = "https://github.com/ggml-org/llama.cpp/releases/download/b11115/llama-b11115-bin-macos-arm64.tar.gz";
      sha256 = "sha256-RS2URpbM12C3vxZDg3wJ9RyYGko0NCl0YGcAfCMmWyU=";
    };
  };
  mise-aarch64-darwin = {
    pname = "mise-aarch64-darwin";
    version = "2026.9.12";
    src = fetchurl {
      url = "https://github.com/jdx/mise/releases/download/v2026.9.12/mise-v2026.9.12-macos-arm64.tar.gz";
      sha256 = "sha256-Dxx/PnTYya6Cl25pkAWPK8aIIazGtMN6aMIGyDZpJBk=";
    };
  };
  pi-aarch64-darwin = {
    pname = "pi-aarch64-darwin";
    version = "0.87.1";
    src = fetchurl {
      url = "https://github.com/earendil-works/pi/releases/download/v0.87.1/pi-darwin-arm64.tar.gz";
      sha256 = "sha256-T40oi3jJdo06Ssb2HwbNNDlLgqwX1bQtHkSkN63UAbc=";
    };
  };
  pi-aarch64-linux = {
    pname = "pi-aarch64-linux";
    version = "0.87.1";
    src = fetchurl {
      url = "https://github.com/earendil-works/pi/releases/download/v0.87.1/pi-linux-arm64.tar.gz";
      sha256 = "sha256-NktKn4SRRQsnpIV9Tjx4Dbr2lnkIIcF2qHPoYMu8O4k=";
    };
  };
  pi-x86_64-linux = {
    pname = "pi-x86_64-linux";
    version = "0.87.1";
    src = fetchurl {
      url = "https://github.com/earendil-works/pi/releases/download/v0.87.1/pi-linux-x64.tar.gz";
      sha256 = "sha256-gNeN1i1QBJoAa5gdmUxhJVvMEOcwsMJ41OoKdVkJdkw=";
    };
  };
  t3-aarch64-linux = {
    pname = "t3-aarch64-linux";
    version = "0.0.42";
    src = fetchurl {
      url = "https://github.com/pingdotgg/t3code/releases/download/v0.0.42/t3-0.0.42-linux-arm64.tar.gz";
      sha256 = "sha256-feR+Znk8kbAe4/swTZ0tyacBmqqPhcVbU11tjKPhQBM=";
    };
  };
  t3-x86_64-linux = {
    pname = "t3-x86_64-linux";
    version = "0.0.42";
    src = fetchurl {
      url = "https://github.com/pingdotgg/t3code/releases/download/v0.0.42/t3-0.0.42-linux-x64.tar.gz";
      sha256 = "sha256-9QTpMe5EBr/nZ1QUfLDioPMA11xsUFoOF0oeefZc7qM=";
    };
  };
  vite-plus-aarch64-darwin = {
    pname = "vite-plus-aarch64-darwin";
    version = "0.3.3";
    src = fetchurl {
      url = "https://github.com/voidzero-dev/vite-plus/releases/download/v0.3.3/vp-aarch64-apple-darwin.tar.gz";
      sha256 = "sha256-hWcToxjxzDszyd+s58ou8Gbxjt6BUPN7f1SQYNRwmDQ=";
    };
  };
  vite-plus-aarch64-linux = {
    pname = "vite-plus-aarch64-linux";
    version = "0.3.3";
    src = fetchurl {
      url = "https://github.com/voidzero-dev/vite-plus/releases/download/v0.3.3/vp-aarch64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-+32rB+RL5J1+/Hhsrug+g0WpkR3iIJlNoLuFFlU4KrU=";
    };
  };
  vite-plus-x86_64-linux = {
    pname = "vite-plus-x86_64-linux";
    version = "0.3.3";
    src = fetchurl {
      url = "https://github.com/voidzero-dev/vite-plus/releases/download/v0.3.3/vp-x86_64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-kQWt4gIA6a7L+dKKUGQNrO97WjOFfFOoOEuAB9So9LQ=";
    };
  };
  zed-preview-x86_64-linux = {
    pname = "zed-preview-x86_64-linux";
    version = "1.21.0-pre";
    src = fetchurl {
      url = "https://github.com/zed-industries/zed/releases/download/v1.21.0-pre/zed-linux-x86_64.tar.gz";
      sha256 = "sha256-6twwCYGWoKISmIpepNGT54ZB4XtFnkJ13ddnUXVEm0Q=";
    };
  };
}
